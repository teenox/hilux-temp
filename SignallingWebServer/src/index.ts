...
program
    ...
    .option(
        ...
        config_file.homepage || 'index.html'
    )
...
const PIXEL_MANAGER_URL = process.env.PIXEL_MANAGER_URL || 'http://localhost:4000';
 
app.post('/pixel-session', (req: any, res: any) => {

    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access

    let sessionId = req.cookies?.pixelSessionId || req.body.pixelSessionId || (req.headers['x-pixel-session-id'] as string);
 
    fetch(`${PIXEL_MANAGER_URL}/instances`, {

        method: 'POST',

        headers: { 'Content-Type': 'application/json' },

        body: JSON.stringify({ pixelStreamingId : sessionId })
        ...
        .then((response) => {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Pixel Manager returned ${response.status}`);
        ...
        .then((data: any) => {
            ...
            sessionId = data.pixelStreamingId;

            // eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-member-access

            res.cookie('pixelSessionId', sessionId, { httpOnly: true, maxAge: 10 * 60 * 1000 });

            // eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-member-access

            res.setHeader('X-Pixel-Session-ID', sessionId);

            // eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-member-access

            res.json({ pixelSessionId: sessionId });
        ...
        .catch((err: any) => {
            ...
            console.error('Failed to start pixel session:', err.message);
            ...
            res.status(404).json({ error: 'Failed to start pixel session', pixelSessionId: sessionId });
